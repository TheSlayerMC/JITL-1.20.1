...
public class CrystalDripstoneClusterFeature extends Feature<DripstoneClusterConfiguration> {
    ...
    private boolean canPlacePool(WorldGenLevel level, BlockPos pos) {
        ...
        if(!blockstate.is(Blocks.WATER) && !blockstate.is(JBlocks.CRYSTALLIZED_DRIPSTONE.get()) && !blockstate.is(JBlocks.POINTED_CRYSTALLIZED_DRIPSTONE.get())) {
    ...
    private boolean canBeAdjacentToWater(LevelAccessor level, BlockPos pos) {
        BlockState blockstate = level.getBlockState(pos);
        return blockstate.is(JBlocks.DEPTHS_GRASS.get()) || blockstate.is(JBlocks.DEPTHS_STONE.get()) || blockstate.is(JBlocks.DEPTHS_DIRT.get()) || blockstate.getFluidState().is(FluidTags.WATER);
    }
